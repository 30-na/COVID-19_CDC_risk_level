...
load("Data/CDC_community_level_county_computed_merged_Medium_With_High_MThreshold.csv")
...
load("Data/CDC_community_level_county_computed_merged_Medium_With_Low_MThreshold.csv")
...
#########################LOW MEDIUM HIGH #################################
load("Data/CDC_community_level_county_computed_MThreshold.csv")
...
fig_consis_rate_line02 = ggplot(data = consis_plot_3,
    ...
    geom_point(alpha = .3)+
    scale_color_manual(values = c("#e34a33", "#2b8cbe", "#fdbb84"))+
    labs(title="Consistancy Rate for each Comunity Risk Level in 3weeks for Modified Threshold")
...
fig_risk_level_proportion02_line = community_level_county_computed %>%
    ...
    geom_smooth(method = "lm",
                formula = y~ poly(x, 15))+
    scale_color_manual(values = c("#e34a33", "#2b8cbe", "#fdbb84"))+
    labs(title="Proportion of Counties in Three Risk Level each week for Modified Threshold")
...
fig_facet_proportion_RL_consisRate = grid.arrange(fig_consis_rate_line02,
                                                  ...
                                                  nrow=2)
ggsave("Result/consisRate_RLProportion_facet_modified_thereshold.jpg",
